...
// convert
    else if(dap_chain_node_cli_find_option_val(argv, l_arg_index, min(argc, l_arg_index + 1), "convert", NULL))
...
// convert
        dap_chain_node_cli_set_reply_text(str_reply,
            "Format of command: wallet {new -w <wallet_name> | list | info [-addr <addr>]|[-w <wallet_name> -net <net_name>]|convert -w <wallet_name> -password <new_password> }");
        return -1;
    }
...
// convert
        case CMD_WALLET_CONVERT:
            int convert_pass = dap_chain_node_cli_find_option_val(argv, l_arg_index, argc, "-password", &l_pass_str);
            
            // check wallet name existence 
            if(!l_wallet_name) {
            ...
            if (convert_pass == 0 || l_pass_str == NULL) {
            ...
            if (!l_exists) {
                dap_chain_node_cli_set_reply_text(str_reply, "Wallet %s not found", l_wallet_name);
                return -1;
            }
            fclose(l_exists);

            // open wallet
            dap_chain_wallet_t * convert_wallet = dap_chain_wallet_open(l_wallet_name, c_wallets_path);
            if (!convert_wallet) {
                dap_chain_node_cli_set_reply_text(str_reply, "Wallet already has password");
                return -1;
            }
            
            // save wallet with password
            if ( !(dap_chain_wallet_save(convert_wallet, l_pass_str)) )
                dap_string_append_printf(l_string_ret, "Wallet successfully converted\n");
            else dap_string_append_printf(l_string_ret, "Wallet is not converted because of internal error\n");

            dap_chain_wallet_close(convert_wallet);
            break;
